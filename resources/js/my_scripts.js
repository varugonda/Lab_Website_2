...
function loadStatsPage(){
	var dataTable = document.getElementById("stats_table");
	var idx;
	var wins = 0;
	var losses = 0;
	var winningTeam;
	for(idx = 2; idx < dataTable.rows.length; idx++ ){
		if(parseInt(dataTable.rows[idx].cells[2].innerHTML) > parseInt(dataTable.rows[idx].cells[3].innerHTML)){
			wins++;
			winningTeam = "CU Boulder";
		}
		else{
			losses++;
			winningTeam = dataTable.rows[idx].cells[1].innerHTML;
		}
		dataTable.rows[idx].cells[4].innerHTML = winningTeam;
	}

	//to update wins and losses
...
function loadPlayersPage(){
	...
	for(idx = 0; idx < players.length; idx++){
		document.getElementById("player_selector").innerHTML += '<a href = "#" onclick = "switchPlayers(' + idx + ')">' + players[idx].name + '</br></a>';
	}
}
